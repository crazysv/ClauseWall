...
export default function DeliberationSummary({
  ...
  if (isLoading) {
    ...
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="p-3 rounded-none bg-amber-500/[0.04] border border-amber-500/15"
      >
        ...
        <p className="text-[10px] text-foreground mt-1">Step {step} of 3</p>
      </motion.div>
    );
  }
  ...
  if (deliberation) {
    const v =
      verdictConfig[deliberation.arbiterVerdict.verdict] ||
      verdictConfig.partially_fair;
    const predConf = Math.round(deliberation.predatorArgument.confidence * 100);
    const guardConf = Math.round(
      deliberation.guardianArgument.confidence * 100,
    );
    ...
    return (
      <motion.div
        ...
        className="p-3 rounded-none bg-white/[0.03] border border-foreground border-2 hover:border-foreground border-2 transition-colors cursor-pointer group"
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-foreground">({arbConf}%)</span>
            </div>
        ...
        {documentId &&
          ...
          (deliberation.arbiterVerdict.verdict === "illegal" ||
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2 pt-2 border-t border-foreground border-2">
              <span className="text-[10px] text-foreground">Next:</span>
              {deliberation.arbiterVerdict.verdict === "illegal" ? (
                <>
                  <Link
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 4 }}
      animate={{ opacity: 1, y: 0 }}
      className="p-3 rounded-none bg-white/[0.02] border border-foreground border-2 hover:border-foreground border-2 transition-colors cursor-pointer group"
    ...
    >
      <div className="flex items-center justify-between">
        <div className="min-w-0">
          ...
          <p className="text-[10px] text-foreground ml-[22px]">
