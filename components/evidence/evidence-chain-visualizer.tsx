      ...
      <div className="border-l-4 border-black ml-4 pl-8 py-2 relative">
        {links.map((link, i) => (
          <div key={link.item_id} className="relative mb-8 last:mb-0">
            ...
            {/* Content block */}
            <div
              className={`border-4 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${link.verified ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-red-100 dark:bg-red-900/30"}`}
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    className={`text-lg font-black uppercase tracking-widest ${link.verified ? "text-emerald-900 dark:text-emerald-300" : "text-red-900 dark:text-red-300"}`}
                  >
                ...
                <span
                  className={`text-xl font-black ${link.verified ? "text-emerald-700 dark:text-emerald-400" : "text-red-700 dark:text-red-400"}`}
                >
                  {link.verified ? "✓" : "✗"}
                </span>
              </div>
