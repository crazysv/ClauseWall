...
export function StorageUsageBar({ usedBytes, limitBytes = 200 * 1024 * 1024 }: { usedBytes: number; limitBytes?: number }) {
  ...
  const color = percent >= 95 ? "bg-red-600" : percent >= 80 ? "bg-yellow-500" : "bg-blue-600";
  const textColor = percent >= 95 ? "text-red-700 dark:text-red-400" : percent >= 80 ? "text-yellow-700 dark:text-yellow-400" : "text-muted-foreground";
  ...
  return (
    <div className="space-y-2">
      ...
      {percent >= 95 && (
        <p className="text-sm font-bold text-red-700 dark:text-red-400 pt-1">STORAGE ALMOST FULL. DELETE OLD BUNDLES TO FREE SPACE.</p>
      )}
    </div>
  );
}
