...
> = {
  ...
  story: {
    width: 1080,
    height: 1920,
    label: "WhatsApp / IG Story",
    icon: Smartphone,
  },
};

// ── Risk-based Card Themes (MUCH DARKER + HIGH CONTRAST) ──
...
function getTopRedFlag(
  ...
  const risky = clauses
    .filter(
      (c) => c.risk_level === "illegal" || c.risk_level === "dangerous"
    )
...
export default function ScoreCardModal({
  ...
  // ── Fetch latest public_share_id when modal opens ──
  useEffect(() => {
    if (isOpen) {
  ...
  // Regenerate when modal opens, format changes, or QR status changes
  useEffect(() => {
    if (isOpen) {
      setPreview(null);
  ...
  }, [isOpen, format, publicShareId, generateImage]);
  ...
  const handleNativeShare = async () => {
    ...
    const file = dataUrlToFile(
      preview,
      `clausewall-scorecard-${doc.id.substring(0, 8)}.png`
    );
  ...
  return (
    <>
        ...
        <DialogContent className="bg-gray-900 border-gray-800 max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          {!hasQR && (
            <p className="text-[10px] text-gray-500 text-center">
              💡 Generate a QR verification badge below to include it on your score card
            </p>
          )}
      ...
      {/* ════════════════════════════════════════ */}
      {/* HIDDEN CARD — Rendered off-screen        */}
      {/* ════════════════════════════════════════ */}
