...
const STATUS_CONFIG = {
  completed: { icon: CheckCircle2, color: "text-green-700 dark:text-green-400", bg: "bg-green-200 dark:bg-green-900", line: "bg-black dark:bg-white" },
  pending: { icon: Clock, color: "text-amber-700 dark:text-amber-400", bg: "bg-amber-200 dark:bg-amber-900", line: "bg-black dark:bg-white border-dashed" },
  overdue: { icon: AlertTriangle, color: "text-red-700 dark:text-red-400", bg: "bg-red-200 dark:bg-red-900", line: "bg-black dark:bg-white" },
  upcoming: { icon: Circle, color: "text-gray-600 dark:text-gray-400", bg: "bg-gray-200 dark:bg-zinc-800", line: "bg-gray-300 dark:bg-gray-700 border-dashed" },
  skipped: { icon: Circle, color: "text-gray-500", bg: "bg-gray-200 dark:bg-zinc-800", line: "bg-gray-300 dark:bg-gray-700" },
};
    ...
    <div className="relative flex gap-4">
      ...
      <div className="flex flex-col items-center">
        <motion.div
          ...
          animate={isActive ? { scale: [1, 1.05, 1], boxShadow: ["2px 2px 0px 0px rgba(0,0,0,1)", "4px 4px 0px 0px rgba(0,0,0,1)", "2px 2px 0px 0px rgba(0,0,0,1)"] } : {}}
          transition={{ duration: 2, repeat: Infinity }}
        >
          <Icon className={`h-5 w-5 stroke-[3px] ${config.color}`} />
        </motion.div>
        {!isLast && (
          <div className={`w-1 flex-1 min-h-[3rem] -mt-2 -mb-2 z-0 border-x-4 ${config.line.includes('border-dashed') ? 'border-dashed border-gray-400 dark:border-gray-600 bg-transparent' : config.line}`} />
        )}
      </div>

      {/* Step Content */}
      <Card className={`flex-1 mb-6 rounded-none ${isActive ? "card-impact bg-blue-50 dark:bg-blue-900/20" : "border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] bg-white dark:bg-zinc-900 opacity-80 border-dashed"}`}>
        <CardContent className="p-4 sm:p-5">
          <div className="flex items-start justify-between mb-2">
            ...
            {step.filing_fee !== undefined && (
              <span className={`text-xs font-bold px-3 py-1 border-2 border-black uppercase tracking-widest ${step.filing_fee === 0 ? "bg-green-200 text-green-800" : "bg-amber-200 text-amber-800"}`}>
