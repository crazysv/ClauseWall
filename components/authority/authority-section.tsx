...
export default function AuthoritySection({
  ...
  const [routingResult, setRoutingResult] = useState<JurisdictionResult | null>(
    null,
  );
  const [escalationPath, setEscalationPath] = useState<EscalationPath | null>(
    null,
  );
  const [loading, setLoading] = useState(false);
  const [activeTab, setActiveTab] = useState<
    "authority" | "escalation" | "legal-aid" | "rti"
  >("authority");
  ...
  const loadRouting = async () => {
    ...
    try {
      ...
      const [routeData, escData] = await Promise.all([
        routeRes.json(),
        escRes.json(),
      ]);
      ...
      <div className="flex flex-wrap gap-2 border-b-4 border-black pb-4">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <Icon
                className={`h-5 w-5 stroke-[3px] ${activeTab === tab.id ? "" : tab.color}`}
              />
      ...
      {loading ? (
        <Card className="card-impact p-12 rounded-none text-center">
          <CardContent className="p-0 flex flex-col items-center justify-center">
            ...
            <p className="font-bold text-lg uppercase tracking-widest animate-pulse">
              Finding Authority...
            </p>
          </CardContent>
        </Card>
            ...
            <div className="space-y-4 pt-4">
              <Card className="card-impact bg-pink-100 dark:bg-pink-900/30 border-pink-500 rounded-none p-6 text-center">
                <CardContent className="p-0">
                  ...
                  <h3 className="font-black text-xl uppercase tracking-widest mb-2 text-pink-700 dark:text-pink-300">
                    Need Free Legal Help?
                  </h3>
                  <p className="font-bold text-pink-900 dark:text-pink-100 mb-6">
                    Check if you qualify for free legal services under LSAA
                    1987.
                  </p>
                  <Button
                    asChild
                    className="btn-impact bg-pink-600 hover:bg-pink-700 text-white w-full sm:w-auto h-auto py-3 px-8 text-sm gap-2"
                  >
                    <a href="/authority/legal-aid">
                      <Heart className="h-5 w-5 stroke-[3px]" /> CHECK
                      ELIGIBILITY
                    </a>
                  </Button>
                </CardContent>
            ...
            <div className="pt-4">
              <Card className="card-impact bg-emerald-100 dark:bg-emerald-900/30 border-emerald-500 rounded-none p-6 text-center">
                <CardContent className="p-0">
                  ...
                  <h3 className="font-black text-xl uppercase tracking-widest mb-2 text-emerald-700 dark:text-emerald-300">
                    File an RTI Query
                  </h3>
                  <p className="font-bold text-emerald-900 dark:text-emerald-100 mb-6">
                    Cost: ₹10 only. Get information from any government body
                    within 30 days.
                  </p>
                  <Button
                    asChild
                    className="btn-impact bg-emerald-600 hover:bg-emerald-700 text-white w-full sm:w-auto h-auto py-3 px-8 text-sm gap-2"
                  >
