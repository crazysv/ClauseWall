...
export default function LegalAidChecker() {
  ...
  const CATEGORIES = [
    ...
    ...Object.entries(LEGAL_AID_CATEGORY_LABELS).map(([k, v]) => ({
      value: k,
      label: v,
    })),
  ];
    ...
    <div className="space-y-6">
      <Card className="card-impact p-8 rounded-none">
        <CardContent className="p-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full border-4 border-black p-3 bg-white dark:bg-zinc-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] font-bold focus:outline-none focus:translate-x-1 focus:translate-y-1 focus:shadow-none transition-all"
              >
                {Object.entries(JURISDICTION_TO_STATE_CODE)
                  .filter(([k]) => k !== "general")
                  .map(([k]) => (
          ...
          <Button
            onClick={handleCheck}
            disabled={loading}
            className="w-full mt-8 btn-impact bg-pink-600 hover:bg-pink-700 text-white py-6"
            id="legal-aid-check"
          >
            {loading ? (
              <Loader2 className="h-6 w-6 animate-spin mr-2" />
            ) : (
              <Heart className="h-6 w-6 mr-2 stroke-[3px]" />
            )}
      ...
      {result && (
        ...
        >
          {/* Eligibility */}
          <Card
            className={`border-4 rounded-none ${result.eligibility.is_eligible ? "border-green-600 bg-green-50 dark:bg-green-900/20 shadow-[4px_4px_0px_0px_rgba(22,163,74,1)]" : "border-amber-500 bg-amber-50 dark:bg-amber-900/20 shadow-[4px_4px_0px_0px_rgba(245,158,11,1)]"}`}
          >
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-black text-lg uppercase tracking-widest">
                  {result.eligibility.is_eligible
                    ? "You are eligible for FREE legal aid!"
            ...
            <div className="pt-4 border-t-4 border-black">
              ...
              <div className="space-y-4">
                {result.providers.slice(0, 8).map((p, i) => (
                  <Card
                    key={i}
                    className="card-impact bg-white dark:bg-zinc-900 rounded-none border-2"
                  >
                      ...
                      {p.phone_numbers?.[0] &&
                        p.phone_numbers[0] !== "[VERIFY]" && (
                          <a
                            ...
                            className="text-sm font-bold text-blue-600 dark:text-blue-400 hover:text-blue-800 flex items-center gap-2 mt-2 w-max bg-blue-50 dark:bg-blue-900/30 px-3 py-1 border-2 border-transparent hover:border-blue-500 transition-colors"
                          >
                            <Phone className="h-4 w-4 stroke-[3px]" />{" "}
                            {p.phone_numbers[0]}
                          </a>
                        )}
                    </CardContent>
          ...
          <div className="pt-4 border-t-4 border-black">
            <h3 className="text-lg font-black uppercase tracking-widest mb-4 flex items-center gap-2">
              <Phone className="h-6 w-6 stroke-[3px]" /> NATIONAL HELPLINES
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {result.helplines.map((h, i) => (
                <a
                  key={i}
                  href={`tel:${h.number}`}
                  className="flex items-center gap-4 p-4 border-4 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-1 hover:shadow-none transition-all bg-green-50 dark:bg-green-900/20 group"
                >
