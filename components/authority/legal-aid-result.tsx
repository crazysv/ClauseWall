...
export function LegalAidResultView({ result }: Props) {
  return (
              ...
              <Card key={i} className="border-white/10 bg-white dark:bg-slate-900/[0.02]">
                <CardContent className="p-3 flex items-center justify-between">
                  ...
                  {p.phone_numbers?.[0] && p.phone_numbers[0] !== "[VERIFY]" && (
                    <a href={`tel:${p.phone_numbers[0]}`} className="p-2 rounded-xl bg-green-500/15 text-green-400 hover:bg-green-500/25">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {result.helplines.slice(0, 4).map((h, i) => (
            <a key={i} href={`tel:${h.number}`} className="flex items-center gap-2 p-2.5 rounded-xl bg-white dark:bg-card/[0.02] border border-white/10 hover:border-green-500/30 transition">
