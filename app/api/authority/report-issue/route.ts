...
export async function POST(request: Request) {
  try {
    ...
    const { authority_id, issue_type, description, suggested_correction } =
      body;
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Missing required fields: authority_id, issue_type, description",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      console.warn("[ClauseWall] Issue report (no table):", {
        authority_id,
        issue_type,
        description,
      });
    ...
    return NextResponse.json({
      success: true,
      message:
        "Thank you for reporting. We will review and update the information.",
    });
  } catch (error) {
