...
export async function POST(request: NextRequest) {
  try {
    ...
    const amount = ocrResult.data?.amount ? `₹${ocrResult.data.amount.toLocaleString("en-IN")}` : "Amount unknown";
    ...
    const result = await addEvidenceItem(caseId, user.id, {
      ...
      source: "ocr_capture",
      captured_at: ocrResult.data?.date ? `${ocrResult.data.date}T00:00:00Z` : undefined,
    });
