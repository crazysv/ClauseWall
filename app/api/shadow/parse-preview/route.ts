...
import {
  parseWhatsAppExport,
  parseWhatsAppZip,
} from "@/lib/shadow/parsers/whatsapp-parser";
import {
  parseEmailText,
  parseEmlFile,
} from "@/lib/shadow/parsers/email-parser";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file) {
      ...
      if (type === "whatsapp_chat") {
        if (format === "zip") {
          ...
          wordCount = chat.messages.reduce(
            (acc, m) => acc + m.message.split(/\s+/).length,
            0,
          );
        } else {
        ...
        const content = await file.text();
        const parsed =
          format === "eml" ? parseEmlFile(content) : parseEmailText(content);
        textPreview = [
    ...
    const message =
      error instanceof Error ? error.message : "Parse preview failed";
