...
export async function POST(req: NextRequest) {
  try {
    ...
    if (doc.analysis_status !== "completed") {
      return NextResponse.json(
        { error: "Document must be fully analyzed first" },
        { status: 400 },
      );
    }
    ...
    const textType =
      (formData.get("evidence_text_type") as EvidenceType) || "other_text";
    ...
    const message =
      error instanceof Error ? error.message : "Shadow analysis failed";
