...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate languages
    if (
      body.preferred_input_language &&
      body.preferred_input_language !== "auto"
    ) {
      if (
        !LANGUAGE_CONFIGS[body.preferred_input_language as SupportedLanguage]
      ) {
        return NextResponse.json(
    ...
    if (body.preferred_output_language) {
      if (
        !LANGUAGE_CONFIGS[body.preferred_output_language as SupportedLanguage]
      ) {
        return NextResponse.json(
      ...
      .upsert(
        {
          ...
          enable_bilingual_by_default: Boolean(
            body.enable_bilingual_by_default,
          ),
