    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-10 md:py-16">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Button variant="outline" size="sm" onClick={() => router.back()} className="border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-none">
        ...
        {routing?.limitation_check?.is_expired && (
          <div className="mb-6 p-4 border-4 border-red-500 bg-red-100 dark:bg-red-900/30 shadow-[4px_4px_0px_0px_rgba(239,68,68,1)]">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-black uppercase tracking-wider text-red-600 dark:text-red-400">Limitation Period Expired</span>
            </div>
          ...
          {step === 0 && (
            <motion.div key="authority" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <Card className="card-impact mb-6 p-6 rounded-none">
                <CardContent className="p-0">
                  ...
                  <div className="flex gap-3">
                    ...
                    />
                    <Button size="sm" onClick={determineAuthority} className="border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] bg-orange-600 hover:bg-orange-700 font-bold uppercase tracking-widest text-white rounded-none">
                      Update
                    </Button>
                  </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          {i === 0 && <span className="text-[10px] px-2 py-1 font-black uppercase tracking-widest bg-orange-500 text-black border-2 border-black">Recommended</span>}
                        ...
                        <h4 className="font-black text-lg uppercase tracking-wide">{rec.primary.name}</h4>
                        <p className="text-sm font-medium text-muted-foreground mt-2">{rec.reasoning}</p>
                      ...
                      <div className={`h-6 w-6 border-4 flex-shrink-0 mt-1 ${
                        selectedAuth?.primary.id === rec.primary.id ? "border-orange-500 bg-orange-500 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" : "border-black bg-white dark:bg-black"
                      }`} />
                    </div>
                      ...
                      <div className="mt-4 pt-4 border-t-4 border-black flex items-center gap-2">
                        ...
                        <span className="text-sm font-black uppercase tracking-wide">
                          Filing Fee: {routing.fee_calculations[i].is_free
                            ? <span className="text-green-500 shadow-sm px-2 py-1 bg-green-100 dark:bg-green-900 border-2 border-green-500">FREE ✓</span>
                            : <span className="text-amber-600 dark:text-amber-400 px-2 py-1 bg-amber-100 dark:bg-amber-900/50 border-2 border-amber-500">₹{routing.fee_calculations[i].filing_fee.toLocaleString('en-IN')}</span>
                          }
                        </span>
                      </div>
                    )}
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <span className="text-sm font-bold uppercase tracking-wide text-muted-foreground">
                          {rec.limitation_period.is_expired
                            ? <span className="text-red-500">Limitation may have expired</span>
              ...
              <div className="flex justify-end mt-8">
                <Button onClick={() => setStep(1)} disabled={!selectedAuth} className="border-4 border-black text-white bg-orange-600 hover:bg-orange-700 hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-8 py-6 text-lg gap-3">
                  Next: Your Details <ArrowRight className="h-6 w-6 stroke-[3px]" />
                </Button>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <Card className="card-impact mb-6 p-6 rounded-none">
                <CardContent className="p-0 space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input value={complainantName} onChange={e => setComplainantName(e.target.value)} placeholder="Full Name (as on ID)" className="bg-white dark:bg-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] px-4 py-3 font-bold focus:border-orange-500 focus:outline-none w-full" />
                    <input value={complainantPhone} onChange={e => setComplainantPhone(e.target.value)} placeholder="Phone Number" className="bg-white dark:bg-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] px-4 py-3 font-bold focus:border-orange-500 focus:outline-none w-full" />
                  </div>
                  <textarea value={complainantAddress} onChange={e => setComplainantAddress(e.target.value)} placeholder="Complete Address (with PIN code)" rows={3} className="w-full bg-white dark:bg-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] px-4 py-3 font-bold focus:border-orange-500 focus:outline-none" />
                </CardContent>
              </Card>
              ...
              <Card className="card-impact mb-6 p-6 rounded-none">
                <CardContent className="p-0 space-y-6">
                  ...
                  <input value={respondentName} onChange={e => setRespondentName(e.target.value)} placeholder="Company / Person Name" className="w-full bg-white dark:bg-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] px-4 py-3 font-bold focus:border-orange-500 focus:outline-none" />
                  <textarea value={respondentAddress} onChange={e => setRespondentAddress(e.target.value)} placeholder="Registered Address" rows={3} className="w-full bg-white dark:bg-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] px-4 py-3 font-bold focus:border-orange-500 focus:outline-none" />
                </CardContent>
              </Card>

              <Card className="card-impact mb-6 p-6 rounded-none">
                <CardContent className="p-0">
                  <h3 className="font-black text-lg uppercase tracking-widest mb-4">Additional Context (Optional)</h3>
                  <textarea value={additionalContext} onChange={e => setAdditionalContext(e.target.value)} placeholder="Any extra details about your complaint..." rows={4} className="w-full bg-white dark:bg-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] px-4 py-3 font-bold focus:border-orange-500 focus:outline-none" />
                </CardContent>
              </Card>

              <div className="flex justify-between mt-8">
                <Button variant="outline" onClick={() => setStep(0)} className="border-4 border-black text-black dark:text-white hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-6 py-6 text-lg gap-3">
                  <ArrowLeft className="h-6 w-6 mr-2 stroke-[3px]" /> Back
                </Button>
                <Button onClick={() => setStep(2)} className="border-4 border-black text-white bg-orange-600 hover:bg-orange-700 hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-8 py-6 text-lg gap-3">
            ...
            <motion.div key="generate" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              {!complaintDocs ? (
                <div className="card-impact p-16 text-center rounded-none">
                  ...
                  <h3 className="text-impact-subheading mb-2">Ready to Generate</h3>
                  <p className="font-bold text-muted-foreground uppercase tracking-widest mb-2">
                    Filing at: <span className="text-black dark:text-white underline decoration-2">{selectedAuth?.primary.short_name}</span>
                  </p>
                  ...
                  <Button onClick={handleGenerate} disabled={generating} className="border-4 border-black text-white bg-orange-600 hover:bg-orange-700 hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-8 py-6 text-lg gap-3">
                    {generating ? <Loader2 className="h-6 w-6 animate-spin" /> : <Gavel className="h-6 w-6 stroke-[3px]" />}
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-8 pb-4 border-b-4 border-black">
                    ...
                    <h3 className="text-impact-subheading">Documents Generated</h3>
                  </div>

                  {[complaintDocs.complaint, complaintDocs.affidavit, complaintDocs.synopsis].filter(Boolean).map(doc => (
                    <Card key={doc!.id} className="card-impact mb-6 p-6 rounded-none">
                      <CardContent className="p-0">
                        <div className="flex items-start justify-between gap-4 mb-4">
                          <div>
                            <h4 className="font-black text-lg uppercase tracking-wide">{doc!.title}</h4>
                            <p className="text-sm font-bold text-muted-foreground mt-1">{doc!.format_notes}</p>
                          </div>
                          <Button variant="outline" size="sm" onClick={() => copyToClipboard(doc!.content, doc!.title)} className="border-2 border-black font-bold uppercase tracking-widest bg-white dark:bg-black hover:bg-gray-100 dark:hover:bg-gray-800 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rounded-none gap-2">
                        ...
                        <details className="group border-t-2 border-black pt-4">
                          <summary className="text-sm font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 cursor-pointer hover:underline list-none ::marker:hidden flex items-center gap-2">
                            View Document Content <ChevronDown className="h-4 w-4 group-open:rotate-180 transition-transform" />
                          </summary>
                      ...
                      <CardContent className="p-0">
                        ...
                        <p className="text-4xl font-black mb-4">
                          {complaintDocs.fee.is_free
                            ? <span className="text-green-500 bg-green-100 dark:bg-green-900 border-4 border-green-500 px-4 py-2 inline-block">FREE ✓</span>
                            : <span className="text-amber-500 bg-amber-100 dark:bg-amber-900/50 border-4 border-amber-500 px-4 py-2 inline-block">₹{complaintDocs.fee.filing_fee.toLocaleString('en-IN')}</span>
                          }
                        </p>
                        <div className="space-y-2">
                          {complaintDocs.fee.notes.map((note, i) => (
                            <p key={i} className="text-sm font-bold text-muted-foreground uppercase flex items-center gap-2 pr-4"><ArrowRight className="h-4 w-4"/> {note}</p>
                          ))}
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  <div className="flex justify-between mt-8">
                    <Button variant="outline" onClick={() => setStep(1)} className="border-4 border-black text-black dark:text-white hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-6 py-6 text-lg gap-3">
                      <ArrowLeft className="h-6 w-6 mr-2 stroke-[3px]" /> Back
                    </Button>
                    <Button onClick={() => setStep(3)} className="border-4 border-black text-white bg-orange-600 hover:bg-orange-700 hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-8 py-6 text-lg gap-3">
                      Next: Filing Guide <ArrowRight className="h-6 w-6 stroke-[3px]" />
                    </Button>
                  </div>
                </div>
              )}
            ...
            <motion.div key="guide" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              {guide ? (
                <div className="space-y-4">
                  ...
                  {guide.steps.map((gs) => (
                    <Card key={gs.step_number} className="card-impact mb-6 p-6 rounded-none">
                      <CardContent className="p-0">
                        <div className="flex items-start gap-6">
                          ...
                          <div className="flex-1">
                            <h4 className="font-black text-lg uppercase tracking-wide mb-2">{gs.title}</h4>
                            <p className="font-medium text-muted-foreground leading-relaxed text-sm mb-4">{gs.description}</p>
                            {gs.url && (
                              <a href={gs.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 font-bold uppercase tracking-widest text-sm text-blue-600 dark:text-blue-400 bg-white dark:bg-black border-2 border-black px-4 py-2 hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] mb-4">
                            ...
                            {gs.tips.length > 0 && (
                              <div className="space-y-2 bg-green-50 dark:bg-green-900/10 border-l-4 border-green-500 p-4">
                                {gs.tips.map((tip, ti) => (
                                  <p key={ti} className="font-bold text-sm text-green-700 dark:text-green-400">💡 {tip}</p>
                                ))}
                              </div>
                            )}
                          </div>
                    ...
                    <CardContent className="p-0">
                      ...
                      <div className="space-y-4">
                        {guide.documents_checklist.map((item, i) => (
                          <div key={i} className="flex items-start gap-4 p-4 border-2 border-gray-200 dark:border-gray-800 bg-white dark:bg-zinc-900 hover:border-black dark:hover:border-white transition-colors">
                            <span className="text-2xl pt-1">
                              {item.available ? '✅' : item.required ? '⚠️' : '📎'}
                            </span>
                            <div>
                              <p className="font-bold text-lg uppercase tracking-wide">{item.document}</p>
                              {item.how_to_get && <p className="font-medium text-sm text-muted-foreground mt-2 border-l-2 border-gray-300 pl-3">{item.how_to_get}</p>}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {guide.helpline && (
                    <div className="p-6 bg-blue-100 dark:bg-blue-900/30 border-4 border-blue-500 text-center shadow-[4px_4px_0px_0px_rgba(59,130,246,1)]">
                      <p className="font-black text-xl uppercase tracking-widest text-blue-700 dark:text-blue-300">
                        📞 Need help? Call: <span className="bg-white dark:bg-black px-4 py-2 border-2 border-blue-500 ml-2 shadow-[2px_2px_0px_0px_rgba(59,130,246,1)] text-black dark:text-white">{guide.helpline}</span>
                      </p>
                    </div>
                  )}

                  <div className="flex justify-between mt-12 pb-16">
                    <Button variant="outline" onClick={() => setStep(2)} className="border-4 border-black text-black dark:text-white hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-6 py-6 text-lg gap-3">
                      <ArrowLeft className="h-6 w-6 mr-2 stroke-[3px]" /> Back
                    </Button>
                    <Link href={`/complaint`}>
                      <Button className="border-4 border-black text-white bg-green-600 hover:bg-green-700 hover:-translate-y-1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-none font-black uppercase tracking-widest px-8 py-6 text-lg gap-3">
                        <CheckCircle2 className="h-6 w-6 stroke-[3px]" /> Done — View All
                      </Button>
                    </Link>
                  </div>
                </div>
